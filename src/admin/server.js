...
app.get('/api/push/status', requireAuth, async (req,res)=>{try{const v=await ensureVapid();res.json({success:true,devices:getSubscriptionStatus(),publicKeyReady:Boolean(v.vapidPublicKey),count:getSubscriptionStatus().length})}catch(e){res.status(500).json({success:false,error:e.message||'Push status unavailable'})}});
