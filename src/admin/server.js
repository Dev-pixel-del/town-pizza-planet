...
app.post('/api/push/subscribe', requireAuth, async (req,res)=>{try{const b=req.body||{};await saveSubscription(b.subscription||b,b);res.json({success:true,devices:getSubscriptionStatus()})}catch(e){res.status(400).json({success:false,error:e.message||'Invalid subscription'})}});
app.delete('/api/push/subscribe', requireAuth, async (req,res)=>{try{await removeSubscription(String(req.body?.endpoint||''));res.json({success:true,devices:getSubscriptionStatus()})}catch(e){res.status(400).json({success:false,error:e.message||'Could not remove subscription'})}});
app.get('/api/push/status', requireAuth, async (req,res)=>{try{const v=await ensureVapid();res.json({success:true,devices:getSubscriptionStatus(),publicKeyReady:Boolean(v.vapidPublicKey),count:getSubscriptionStatus().length,externalAlarmConfigured:Boolean(String(process.env.WEBHOOK_NOTIFIER_URL||'').trim()&&String(process.env.WEBHOOK_NOTIFIER_SECRET||'').trim())})}catch(e){res.status(500).json({success:false,error:e.message||'Push status unavailable'})}});
...
app.post('/api/push/test-webhook', requireAuth, async (req,res)=>{try{const result=await require('../ownerAlerts').testWebhookNotifier();res.json({success:true,...result})}catch(e){res.status(500).json({success:false,error:e.message||'Webhook test failed'})}});
...
app.put('/api/orders/:orderId/status', requireAuth, async (req,res)=>{
  ...
  const before=getOrderById(req.params.orderId); if(!before)return res.status(404).json({success:false,error:'Order not found'});
  const meta=getOrderMeta(before.order_id)||{}; const alert=meta.ownerAlert||{};
  if(status!=='received' && status!=='cancelled' && alert.state!=='acknowledged'){
    return res.status(409).json({success:false,code:'OWNER_CONFIRMATION_REQUIRED',error:'Owner must explicitly confirm this order from the Order Alert before moving it to Preparing.'});
  }
  if(status===before.status) return res.json({success:true,changed:false,order:decorateOrder(before)});
  ...
  await appendAudit('order.status_changed',{orderId:order.order_id,from:before.status,to:status});
...
app.get('/api/qr.png',(req,res)=>{
  res.setHeader('Cache-Control','no-store, no-cache, must-revalidate');
  const age = whatsappState.qrCreatedAt ? Date.now() - whatsappState.qrCreatedAt : Infinity;
  const dataUrl = age <= 90000 ? whatsappState.qrDataUrl : null;
  if (!dataUrl || typeof dataUrl !== 'string') return res.status(404).type('text/plain').send('QR not available');
  const m = dataUrl.match(/^data:image\/(png|jpeg);base64,(.+)$/);
  if (!m) return res.status(500).type('text/plain').send('Invalid QR data');
  res.type(m[1] === 'jpeg' ? 'image/jpeg' : 'image/png').send(Buffer.from(m[2], 'base64'));
});
