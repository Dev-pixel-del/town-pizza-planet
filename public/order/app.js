...
const I18N = {
  en: {
    ...
    orderLookupFailed:'Could not load this order.', retry:'Try again', thanks:'Thank you for ordering from Town Pizza Planet!', playVoiceConfirmation:'🔊 Play voice confirmation', voiceConfirmationPlaying:'🔊 Playing delivery confirmation…', voiceConfirmationReady:'Delivery confirmation',
  ...
  kn: {
    ...
    orderLookupFailed:'ಆರ್ಡರ್ ಲೋಡ್ ಆಗಲಿಲ್ಲ.', retry:'ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ', thanks:'ಟೌನ್ ಪಿಜ್ಜಾ ಪ್ಲಾನೆಟ್ ಆಯ್ಕೆ ಮಾಡಿದಕ್ಕಾಗಿ ಧನ್ಯವಾದಗಳು!', playVoiceConfirmation:'🔊 ಧ್ವನಿ ದೃಢೀಕರಣ ಕೇಳಿ', voiceConfirmationPlaying:'🔊 ಡೆಲಿವರಿ ಧ್ವನಿ ಕೇಳಿಸಲಾಗುತ್ತಿದೆ…', voiceConfirmationReady:'ಡೆಲಿವರಿ ಧ್ವನಿ ದೃಢೀಕರಣ',
  ...
  hi: {
    ...
    orderLookupFailed:'ऑर्डर लोड नहीं हो सका।', retry:'फिर कोशिश करें', thanks:'टाउन पिज़्ज़ा प्लैनेट से ऑर्डर करने के लिए धन्यवाद!', playVoiceConfirmation:'🔊 आवाज़ में पुष्टि सुनें', voiceConfirmationPlaying:'🔊 डिलीवरी की पुष्टि सुनाई जा रही है…', voiceConfirmationReady:'डिलीवरी आवाज़ पुष्टि',
  ...
  ur: {
    ...
    orderLookupFailed:'آرڈر لوڈ نہیں ہوسکا۔', retry:'دوبارہ کوشش کریں', thanks:'ٹاؤن پیزا پلینیٹ سے آرڈر کرنے کا شکریہ!', playVoiceConfirmation:'🔊 آواز میں تصدیق سنیں', voiceConfirmationPlaying:'🔊 ڈیلیوری کی تصدیق سنائی جا رہی ہے…', voiceConfirmationReady:'ڈیلیوری آواز کی تصدیق',
...
function comboOfDayCard(){
  ...
  const comboDayButton = inCart
    ? `<button class="primary-btn wide combo-day-added-button" data-add-combo-day>✓ ${escapeHtml(t('added'))} × ${inCart.qty}</button>`
    : `<button class="primary-btn wide" data-add-combo-day>Add to Cart</button>`;
  return `<section class="combo-day-card"><div class="combo-day-badge">⭐ COMBO OF THE DAY · 10% OFF</div><div class="combo-day-inner"><div class="combo-day-media"><img src="${c.packType==='family'?packImg(c.image,'family'):packImg(c.image,'combo')}" alt="${escapeHtml(c.name)}"></div><div class="combo-day-copy"><p class="eyebrow">TODAY'S SPECIAL</p><h2>${escapeHtml(c.name)}</h2><p>${escapeHtml(c.description||'')}</p><div class="combo-day-price"><del>${money(c.originalPrice)}</del><strong>${money(c.price)}</strong><span>Save ${money(c.discount)}</span></div>${sold?`<button class="disabled-btn wide" disabled>${escapeHtml(t('soldOut'))}</button>`:comboDayButton}</div></div></section>`;
}
...
function getVoiceConfirmationConfig(order){
  ...
  const files={
    ...
    hi:{30:'hi_30.mp3',45:'hi_45.mp3'},
    ur:{30:'ur_30.mp3',45:'ur_45.mp3'}
  };
  return {lang,minutes,src:`/order/audio/${files[lang]?.[minutes]||files.en[minutes]}`};
}
function setupVoiceConfirmation(order){
  const audio=document.getElementById('tppVoiceConfirmation');
  ...
  // Try automatic playback after the successful order navigation. Some browsers block this;
  // the visible button remains the guaranteed fallback.
  setTimeout(async()=>{
